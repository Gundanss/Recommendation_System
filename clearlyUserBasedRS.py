...
##仅用来使用别人下载的dataset，因为不知道为啥，columns和rows多读了一行: 这个数据集的movie从0开始数，test_rating数据集movie由1开始数
## Item_basd RS
##
...
def predictRating(neighbor_size, sortlist, average, ratings_pivotNan, movie_example):
    # print(average)
    # print("bone average:", average[user_example])
    down = 0
    up = 0
    for i in range(neighbor_size):
        down += sortlist[i][1]
        # print("current rating:", ratings_pivotNan.loc[sortlist[i][0]][movie_example])
        # print("average rating:", average[sortlist[i][0]])
        up += sortlist[i][1] * (ratings_pivotNan.loc[sortlist[i][0]][movie_example] - average[sortlist[i][0]])   ## similarity*(current_rating - average_rating)
    # print("down:", down)
    # print("up:", up)
    predict_rating = average[user_example] + (up / down)
...
# movie data file
dataset = int(input("If use IMDb dataset, input 0. use Douban dataset, input 1:"))
...
if dataset == 1:
    ...
    # ratings data file
    ratings = pd.read_csv('./ez_douban/ratings.csv')
    print('用户数据：%d' % ratings.userId.unique().shape[0])
    print('电影数据：%d' % ratings.movieId.unique().shape[0])
    print('评分数目：%d' % ratings.shape[0])
    print(ratings)
...
print("remained user list:")
print(ratings['userId'].unique())
print("remained movie list:")
...
for movie_example in unseen_movie:
    ...
    average = ratings_pivotNan.mean(1)
    # print("average:")
    # print(average)
    # print("\n")
    ...
    if similarity == 1:
        ...
        bones_ratings_for_pearson = ratings_pivot.loc[user_example]
        # print("cancel all the NaN:")
        # print(ratings_pivot)
        # print(bones_ratings_for_pearson)
        similar_to_bones = ratings_pivot.corrwith(bones_ratings_for_pearson, axis=1, method='pearson')
        pearson_dict = {}
    ...
    ## cosine similarity
    if similarity == 0:
        drop_column = ratings_pivotNan.drop([movie_example], axis=1)
        bones_ratings = drop_column.loc[user_example]
        # bones_ratings = ratings_pivotNan.loc[user_example].drop([user_example])
        # print("bones_ratings:")
        # print(bones_ratings)
        # print("\n")
        ## remove the row that have Nan value
        rating_remove_column = drop_column
        ...
        # print(ratings_pivot.index.values)
        for userid in ratings_pivot.index.values:
            if userid == user_example:
                continue
            dict[userid] = 1 - cosine(rating_remove_column.loc[userid], bones_ratings)
    ...
    predict_dict[movie_example] = predictRating(neighbor_size, sortlist, average, ratings_pivotNan, movie_example)
...
print(recommend_movies_summary.head(11))
Time_end = time.time()
print("time cost:", Time_end - Time_start)
recommend_movies_summary.to_csv('./data/begin/user_based_recommendation.csv')
